              ...
              <Col :xs="24" :sm="16" :md="12" :lg="8" class="input-item">
                <Form ref="form"
                  ...
                  <FormItem label="所属部门：" prop="group">
                    <Select v-model="formData.group" clearable>
  ...
  methods: {
    ...
    handleSumbit() {
      this.$refs.form.validate((valid) => {
        ...
        this.formData.gid = this.formData.group * 1;

        http.Post('Admin.Auth.Role', this.formData)
        .then((res) => {
          if (res.code === 0) {
            const data = res.data;
            if (data.rid) {
    ...
    handleAuthSumbit() {
      ...
      http.Post('Admin.Config.Auth', reqData)
      .then((res) => {
        if (res.code === 0) {
          this.$Notice.success({
            title: res.data,
